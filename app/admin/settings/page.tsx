...
export default function AdminSettings() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchSettings(); // re-fetch to get latest
  ...
  return (
    <AdminLayout wallet={address}>
      ...
      {error && (
        <div className="admin-card border border-red-500/50 text-red-300 p-3 rounded-lg mb-4">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="admin-card border admin-border p-4 sm:p-6 rounded-xl space-y-4 max-w-2xl">
        ...
        <div className="border-t admin-border pt-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button
              ...
              className="admin-accent-bg text-slate-900 px-4 py-2 rounded-lg font-semibold hover:opacity-90 transition text-sm sm:text-base"
            >
              Add
            </button>
          </div>
        ...
        <button
          type="submit"
          className="admin-accent-bg text-slate-900 px-4 sm:px-6 py-2 rounded-lg font-semibold transition hover:opacity-90 text-sm sm:text-base"
          disabled={saving}
        >
