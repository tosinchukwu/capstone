        ...
        <AdminLayout wallet={address}>
            ...
            <form onSubmit={handleSubmit} className="bg-slate-800/70 backdrop-blur-sm p-6 rounded-xl border border-gold-500/20 space-y-4 max-w-2xl">
                ...
                <div className="border-t border-gold-500/20 pt-4">
                    <h3 className="text-lg font-semibold text-gold-300 mb-2">Admin Wallets</h3>
                    <div className="flex gap-2">
                        ...
                        />
                        <button type="button" onClick={addAdminWallet} className="bg-gold-500 text-slate-900 px-4 py-2 rounded-lg font-semibold hover:bg-gold-600 transition">
                            Add
                        </button>
                    </div>
                    <div className="mt-2 space-y-1">
                        {(settings.adminWallets || []).map((w: string) => (
                            <div key={w} className="flex justify-between items-center bg-slate-700/50 px-3 py-1 rounded">
                                <span className="text-sm text-gray-300 truncate">{w}</span>
                                <button type="button" onClick={() => removeAdminWallet(w)} className="text-red-400 hover:text-red-300 text-sm">
                                    Remove
                                </button>
                            </div>
                        ))}
                ...
                <button type="submit" className="bg-gold-500 hover:bg-gold-600 text-slate-900 px-6 py-2 rounded-lg font-semibold transition">
                    Save Settings
                </button>
            </form>
        </AdminLayout>
